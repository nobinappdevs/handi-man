...
export function CartDrawer() {
  ...
  /* Docked at `end` — the right edge in ltr, the left edge in rtl. The
     closed state has to slide off THAT same edge, so the sign flips with it;
     a hardcoded positive offset would land the "closed" drawer back on
     screen once `end` flips sides. */
  const rtl = dir === "rtl";
  ...
  return (
    <aside
      ...
      aria-hidden={!open}
      className={`fixed inset-y-0 end-0 z-50 flex w-[min(380px,88vw)] flex-col bg-drawer text-drawer-ink transition-transform duration-[320ms] ease-[cubic-bezier(.4,0,.2,1)] ${
